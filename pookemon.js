...
class Dresseur {
     ...
     */
    constructor(nom, pokemons, inventaire) {
        this.nom = nom;
    ...
    */
    ...
    ajouterObjet(objets) {
        objets.forEach(objet => {
            var _a;
            (_a = this.inventaire) === null || _a === void 0 ? void 0 : _a.push(objet);
        });
    }
    /**
     ...
     * @returns void
     */
    utiliserObjet(objet, cible) {
        var _a;
        if (((_a = this.inventaire) === null || _a === void 0 ? void 0 : _a.length) == undefined) {
...
class Objets {
    ...
    utiliser(objet, cible) {
        //
    }
}
/* Notre personnage avec ses objets */
const pikachu = new Pokemon('pikachu', 100, 35, 15, (nomSort, cible) => {
    return pikachu.attaqueSpecial(nomSort, cible);
});
const magicarpe = new Pokemon('magicarpe', 70, 15, 15, (nomSort, cible) => {
    return magicarpe.attaqueSpecial(nomSort, cible);
});
const maxipotion = new Objets('maxipotion', (objet, cible) => {
    console.log('la maxipotion restaure 50pv du pokemon choisi');
});
const ranimer = new Objets('ranimer', (objet, cible) => {
    console.log('la maxipotion restaure 50pv du pokemon choisi');
});
/* Le fameux rival */
const grotadmorv = new Pokemon('grotadmorv', 100, 50, 15, (nomSort, cible) => {
    return grotadmorv.attaqueSpecial(nomSort, cible);
});
const racaillou = new Pokemon('racaillou', 70, 60, 15, (nomSort, cible) => {
    return racaillou.attaqueSpecial(nomSort, cible);
});
const maxipotion2 = new Objets('maxipotion', (objet, cible) => {
    console.log('la maxipotion restaure 50pv du pokemon choisi');
});
